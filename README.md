...
- This uses the defconfig with config `CONFIG_TC` disabled and `CONFIG_STATIC` enabled, and with musl.cc's i486 linux musl cross toolchain from x86_64 then upx'ed (unless it's x86_64, in which it is just the bare metal toolchain then upx'ed)
...
- This uses the defconfig `i386_defconfig` for x86; txzboot's kernel for x86_64.

To use the embedded Busybox/Linux seperatly, run the command `temu https://boot.tarxz.zip/`
